...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Warning threshold: {:.0}%",
        buffer_config.warn_threshold * 100.0
    );
    ...
    let mutation = MutationConfig::uniform(0.001)?;
    ...
    let mut sim =
        centrevo::simulation::Simulation::new(structure, mutation, recombination, fitness, config)?;
    ...
    for generation in 0..100 {
        ...
        if generation % 10 == 0 {
            ...
            let status = if recorder.is_buffer_high() {
                "⚠️ HIGH"
            } else {
                "✓ OK"
            };
