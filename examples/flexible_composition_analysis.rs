...
use centrevo::genome::{Chromosome, Haplotype, Individual, RepeatMap};
...
fn main() {
    ...
    println!(
        "   Individual 0, Haplotype 0, Chromosome 0: GC = {:.2}%",
        chr_gc * 100.0
    );
...
fn create_sequence(nucleotides: &[Nucleotide]) -> Sequence {
    let mut seq = Sequence::with_capacity(nucleotides.len());
