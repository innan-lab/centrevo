...
use centrevo::simulation::{
    ...
    FitnessConfig, SimulationConfig, Simulation,
};
...
fn test_resume_preserves_rng_state() {
    ...
    let final_pop_continuous = {
        ...
        // Get final state
        sim.population().individuals()[0].haplotype1().get(0).unwrap()
            .sequence().indices().to_vec()
    };
    ...
    // Count differences
    let differences: usize = final_pop_continuous.iter()
        .zip(final_pop_resumed.iter())
        .filter(|(a, b)| a != b)
        .count();
    
    println!("Differences between continuous and resumed: {} / {}", 
             differences, final_pop_continuous.len());
