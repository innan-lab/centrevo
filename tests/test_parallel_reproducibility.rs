...
use centrevo::simulation::{Simulation, RepeatStructure, MutationConfig, RecombinationConfig, FitnessConfig, SimulationConfig};
...
fn test_parallel_reproducibility() {
    ...
    for (ind1, ind2) in results1.iter().zip(results2.iter()) {
        ...
        for (chr1, chr2) in ind1.haplotype1().chromosomes().iter()
            .zip(ind2.haplotype1().chromosomes().iter()) {
            assert_eq!(chr1.sequence().to_string(), chr2.sequence().to_string(),
                "Haplotype1 sequences differ");
        }
        
        // Compare haplotype2 sequences
        for (chr1, chr2) in ind1.haplotype2().chromosomes().iter()
...
fn test_parallel_different_seeds() {
    ...
    assert!(different_found, "Simulations with different seeds should produce different results");
}
